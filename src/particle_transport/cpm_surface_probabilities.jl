"""
    cpm_legendre_polynomial_coefficients(ℓ::Int64,::Type{T}=Float64) where {T<:AbstractFloat}
...
"""
function cpm_legendre_polynomial_coefficients(ℓ::Int64,::Type{T}=Float64) where {T<:AbstractFloat}
    c = zeros(T,ℓ+1)
...
"""
    cpm_shifted_legendre_polynomial_coefficients(ℓ::Int64,::Type{T}=Float64) where {T<:AbstractFloat}
...
"""
function cpm_shifted_legendre_polynomial_coefficients(ℓ::Int64,::Type{T}=Float64) where {T<:AbstractFloat}
    c = zeros(T,ℓ+1)
    for k in range(0,ℓ)
...
"""
    cpm_polynomial_product(a::Vector{T},b::Vector{T}) where {T<:AbstractFloat}

Monomial coefficients of the product of two polynomials given by their monomial coefficients
(discrete convolution).
"""
function cpm_polynomial_product(a::Vector{T},b::Vector{T}) where {T<:AbstractFloat}
...
"""
    cpm_ray_reduction(c::Vector{T},Σi::T,Δxi::T,Fout::T,w::Int64) where {T<:AbstractFloat}
...
"""
function cpm_ray_reduction(c::Vector{T},Σi::T,Δxi::T,Fout::T,w::Int64) where {T<:AbstractFloat}
    s = zero(T)
    if Σi > 0
        Fin = Fout + Σi*Δxi
        for r in range(0,length(c)-1)
            if c[r+1] != 0
                s += c[r+1]*(cpm_exponential_integral(r+w+2,Fout) - cpm_exponential_integral(r+w+2,Fin))
            end
        end
        return s/Σi
    else
        for r in range(0,length(c)-1)
            if c[r+1] != 0
                s += c[r+1]*cpm_exponential_integral(r+w+1,Fout)
            end
        end
        return Δxi*s
    end
...
"""
    cpm_surface_matrices(Σ::Vector{Float64},Δx::Vector{Float64},Lp::Int64,Nν::Int64)

Assemble the boundary-coupling probability matrices of the 1D CPM in Radiant's half-range
...
The volume moments are indexed as `(i-1)(Lp+1)+p`, matching [`cpm_collision_matrix`](@ref). Each
angular integral over ``\\mu \\in [0,1]`` is evaluated in closed form as a finite combination of
generalized exponential integrals ``E_n`` (see [`cpm_ray_reduction`](@ref)).
...
"""
function cpm_surface_matrices(Σ::Vector{Float64},Δx::Vector{Float64},Lp::Int64,Nν::Int64)
    if Nν ≤ 8
        return cpm_surface_matrices_impl(Σ,Δx,Lp,Nν,Float64)
    else
        # Extra bits ≈ log2(4^{2Nν}) = 4Nν for the worst (transmission) product, plus a margin.
        return setprecision(BigFloat, 64 + 8*Nν) do
            cpm_surface_matrices_impl(Σ,Δx,Lp,Nν,BigFloat)
        end
    end
end

function cpm_surface_matrices_impl(Σ::Vector{Float64},Δx::Vector{Float64},Lp::Int64,Nν::Int64,::Type{T}) where {T<:AbstractFloat}
    Nx = length(Σ)
    P = Lp+1
    Sv = Nν+1
    NS = 2*Sv
    Ftot = T(cpm_optical_thickness(Σ,Δx,1,Nx))

    # Monomial coefficients of the volume (Legendre) and surface (shifted Legendre) bases in T.
    Pcoef = [cpm_legendre_polynomial_coefficients(p-1,T) for p in range(1,P)]                          # P_{p-1}(μ)
    Rcoef = [sqrt(T(2*l+1)) .* cpm_shifted_legendre_polynomial_coefficients(l,T) for l in range(0,Nν)] # R̄_ℓ(μ)
    ...
    for i in range(1,Nx)
        FL = T(cpm_optical_thickness(Σ,Δx,1,i-1))
        FR = T(cpm_optical_thickness(Σ,Δx,i+1,Nx))
        Σi = T(Σ[i]); Δxi = T(Δx[i])
        for p in range(1,P), l in range(0,Nν)
            base = cpm_polynomial_product(Pcoef[p],Rcoef[l+1])       # P_p(μ) R̄_ℓ(μ)
            Pvs[vidx(i,p),sidx(1,l)] = (2π/Δx[i])*cpm_ray_reduction(base,Σi,Δxi,FL,1)
            # Right face : the incoming ray travels toward -x, P_p(-μ) = (-1)^{ℓp} P_p(μ).
            Pvs[vidx(i,p),sidx(2,l)] = (-1)^(p-1)*(2π/Δx[i])*cpm_ray_reduction(base,Σi,Δxi,FR,1)
        end
    end

    # Psv : volume source moment → outgoing boundary moment (1/μ streaming cancels one power, w = 0).
    for j in range(1,Nx)
        FL = T(cpm_optical_thickness(Σ,Δx,1,j-1))
        FR = T(cpm_optical_thickness(Σ,Δx,j+1,Nx))
        Σj = T(Σ[j]); Δxj = T(Δx[j])
        for q in range(1,P), l in range(0,Nν)
            base = cpm_polynomial_product(Rcoef[l+1],Pcoef[q])       # R̄_ℓ(μ) P_q(μ)
            # Left face : outgoing toward -x uses P_q(-μ) = (-1)^{ℓq} P_q(μ) ; right : P_q(μ).
            Psv[sidx(1,l),vidx(j,q)] = (2(q-1)+1)/(4π)*(-1)^(q-1)*cpm_ray_reduction(base,Σj,Δxj,FL,0)
            Psv[sidx(2,l),vidx(j,q)] = (2(q-1)+1)/(4π)*cpm_ray_reduction(base,Σj,Δxj,FR,0)
        end
    end
    ...
    for l in range(0,Nν), lp in range(0,Nν)
        g = cpm_polynomial_product(Rcoef[l+1],Rcoef[lp+1])
        t = zero(T)
        for r in range(0,length(g)-1)
            if g[r+1] != 0 t += g[r+1]*cpm_exponential_integral(r+2,Ftot) end
        end

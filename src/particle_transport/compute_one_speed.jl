        ...
        if false #### PN
            if ndims == 1
                ...
                for ix in range(1,Ns[1]), is in range(1,Nm[5]), p in range(1,Np),q in range(1,Np)
                    Q⁺[p,is,ix,1,1] += (2*(q-1)+1)/2 * Ql[q,is,ix,1,1] * Mll[q,p]
                    Q⁻[p,is,ix,1,1] += (2*(q-1)+1)/2 * (-1)^(q-1) * Ql[q,is,ix,1,1] * Mll[q,p]
                end

                𝚽⁺ = zeros(Np,Nm[5],Ns[1],Ns[2],Ns[3])
                𝚽⁻ = zeros(Np,Nm[5],Ns[1],Ns[2],Ns[3])

                𝚽⁺ = pn_sweep_1D(1,𝚽⁺[:,:,:,1,1],Q⁺[:,:,:,1,1],Σt,mat[:,1,1],Ns[1],Δs[1],Np,Np_surf,𝒪,C,ω,sources)
                𝚽⁻ = pn_sweep_1D(-1,𝚽⁻[:,:,:,1,1],Q⁻[:,:,:,1,1],Σt,mat[:,1,1],Ns[1],Δs[1],Np,Np_surf,𝒪,C,ω,sources)

                for ix in range(1,Ns[1]), is in range(1,Nm[5]), p in range(1,Np), q in range(1,Np)
                    𝚽l[q,is,ix,1,1] += Mll[q,p] * (𝚽⁺[p,is,ix,1,1] + (-1)^(q-1)*𝚽⁻[p,is,ix,1,1])
                end
            end
        else #### SN
            for n in range(1,Nd)
